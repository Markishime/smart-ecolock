...
void logInstructor(String uid, String timestamp, String action) {
  ...
  } else if (action == "EndSession" && uid == lastInstructorUID) {
...
void logStudentToRTDB(String rfidUid, String timestamp, int sensorIndex, String status, String timeOut) {
  ...
  if (sensorIndex >= 0 && sensorIndex < NUM_SENSORS) {
    if (loadCells[sensorIndex]->update() == 2) {  // 2 means stable value available
      float weight = loadCells[sensorIndex]->getData();
      if (abs(weight) < 20) {
        weight = 0.0;
        Serial.println("Weight drift filtered for UID: " + rfidUid + ", Sensor " + String(sensorIndex + 1));
      }
      sensorConfirmed = (weight >= 20);
      if (sensorConfirmed) {
        Serial.println("Sensor " + String(sensorIndex + 1) + ": Present, Weight: " + String(weight) + "g");
      } else {
        Serial.println("No presence detected on Sensor " + String(sensorIndex + 1));
      }
    } else {
  ...
  String action = sensorConfirmed ? "Confirmed" : "Not Confirmed";
  if (status == "Pending" && sensorConfirmed) {
    finalStatus = "Present";
  } else if (status == "Pending" && studentVerificationActive && millis() - studentVerificationStartTime >= Student_VERIFICATION_WINDOW) {
    finalStatus = "Absent";
  } else if (tapOutPhase && timeOut != "") {
    finalStatus = "Absent";
  }
  ...
  // Log to SD
  String sensorStr = (sensorIndex >= 0 && sensorConfirmed) ? "Sensor " + String(sensorIndex + 1) : "None";
  ...
  if (!sdMode && isConnected && Firebase.ready()) {
    ...
    json.set("Status", finalStatus);
    json.set("Time In", timestamp);
    json.set("Time Out", timeOut);
    json.set("Sensor", sensorStr);
    json.set("assignedSensorId", sensorIndex >= 0 ? sensorIndex : -1);
    json.set("weight", weight);
    json.set("weightUnit", "g");
    json.set("sensorConfirmed", sensorConfirmed);
  ...
  if (!sdMode && isConnected && Firebase.ready()) {
    ...
    if (isEntry) {
      ...
      if (assignedRoomId != "" && firestoreRooms.find(assignedRoomId) != firestoreRooms.end()) {
        ...
        // Set status to "maintenance" for admin inspection
    ...
    if (!isEntry && isVoltageSufficient) {
      ...
      if (Firebase.RTDB.getJSON(&fbdo, "/AccessLogs/" + uid)) {
        ...
        FirebaseJsonData entryData;
        FirebaseJsonData actionData;
        String latestEntryKey = "";
        
        size_t count = json.iteratorBegin();
        for (size_t i = 0; i < count; i++) {
          json.iteratorGet(i, entryData);
          if (entryData.type == "object") {
            String entryKey = entryData.key.c_str();
            FirebaseJson entryJson;
            json.get(entryData, entryJson);
            if (entryJson.get(actionData, "action") && actionData.to<String>() == "entry") {
              if (latestEntryKey == "" || entryKey.compareTo(latestEntryKey) > 0) {
      ...
      // If we found the entry record, update it with exit info and PZEM data
      if (entryTimestamp != "") {
        FirebaseJson exitData;
        exitData.set("action", "exit");
        exitData.set("timestamp", timestamp);
        
        // Create PZEM data object with the exact structure requested
        FirebaseJson pzemDataJson;
        pzemDataJson.set("voltage", lastVoltage);
        pzemDataJson.set("current", lastCurrent);
        pzemDataJson.set("power", lastPower);
        pzemDataJson.set("energy", lastEnergy);
        
        // Add PZEM data to the exit object
        exitData.set("pzemData", pzemDataJson);
        exitData.set("totalConsumption", String(lastEnergy, 2) + " kWh");
        
        // Update the roomDetails of the entry record with the exit data
        if (Firebase.RTDB.updateNode(&fbdo, "/AccessLogs/" + uid + "/" + entryTimestamp + "/roomDetails/exit", &exitData)) {
          Serial.println("Exit data updated in entry record: " + entryTimestamp);
        } else {
          Serial.println("Failed to update exit data: " + fbdo.errorReason());
        }
      }
      
      // Also add PZEM data to the current exit record
...
void loop() {
  ...
  if (studentVerificationActive) {
    ...
    if (millis() - lastUpdate >= 1000) {
      ...
      if (pendingStudentTaps.size() > 0) {
        if (currentStudentQueueIndex < pendingStudentTaps.size()) {
          // Show which student we're waiting for verification
          ...
          if (firestoreStudents.find(currentStudentUid) != firestoreStudents.end()) {
            ...
            if (studentName.length() > 10) {
              studentName = studentName.substring(0, 9) + "."; // Truncate long names
            }
          } else {
            studentName = "Student"; // Default if name not found
          }
          
          // Show student number in queue (1-based index)
          int studentQueueNumber = currentStudentQueueIndex + 1;
          studentInfo = "Student " + String(studentQueueNumber) + "/" + String(pendingStudentTaps.size());
          
          // Display with animated dots
          String dots = "";
          for (int i = 0; i < dotCount; i++) dots += ".";
          displayMessage("Awaiting " + studentInfo, studentName + dots, 0);
        } else {
          // All students verified
          displayMessage("Student Check", "Complete" + String(dotCount), 0);
        }
      } else {
        // No students have tapped yet
        String waitMsg = "Students";
        String dots = "";
        for (int i = 0; i < dotCount; i++) dots += ".";
        displayMessage("Awaiting " + waitMsg, "Tap RFID Cards" + dots, 0);
      }
    ...
    // Monitor assigned sensors for presence changes
    for (int i = 0; i < NUM_SENSORS; i++) {
      if (loadCells[i]->update() == 2) {  // 2 means stable value available
        float weight = loadCells[i]->getData();
        if (abs(weight) < 20) weight = 0.0;
        
        // Check if this weight sensor corresponds to current student in queue
        ...
        if (!uid.isEmpty()) {
          ...
          String path = "/Students/" + uid + "/Attendance/" + currentSessionId + "/Status";
          String currentStatus = "Pending";
          if (!sdMode && isConnected && Firebase.ready()) {
            if (Firebase.RTDB.getString(&fbdo, path)) {
              currentStatus = fbdo.to<String>();
            }
          }
          bool prevPresent = (currentStatus == "Present");
          if (prevPresent != nowPresent) {
            String timestamp = getFormattedTime();
            logStudentToRTDB(uid, timestamp, nowPresent ? i : -1, nowPresent ? "Present" : "Pending", "");
          }
        }
      }
    }
    
    // Handle RFID taps during verification
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
      String uidStr = getUIDString();
      String timestamp = getFormattedTime();
      
      // Reset student tap timeout
      lastStudentTapTime = millis();
      
      // Check if this is a student in the verification system
      if (firestoreStudents.find(uidStr) != firestoreStudents.end()) {
        // Check if already in queue
        if (std::find(pendingStudentTaps.begin(), pendingStudentTaps.end(), uidStr) == pendingStudentTaps.end()) {
          // Add to queue if not already present
          pendingStudentTaps.push_back(uidStr);
          Serial.println("Student " + uidStr + " added to verification queue.");
          ...
          if (studentName.length() > 10) studentName = studentName.substring(0, 10);
          displayMessage(studentName, "Added to Queue", 2000);
        } else {
          // Already in queue
          Serial.println("Student " + uidStr + " already in verification queue.");
          displayMessage("Already Tapped", "Please Seat", 2000);
        }
      } else if (firestoreTeachers.find(uidStr) != firestoreTeachers.end() && 
                 uidStr == lastInstructorUID) {
        // Instructor can skip verification
        studentVerificationActive = false;
        classSessionActive = true;
        classSessionStartTime = millis();
        ...
        for (const auto& pendingUid : pendingStudentTaps) {
          if (std::find_if(studentAssignedSensors.begin(), studentAssignedSensors.end(),
                           [&](const auto& pair) { return pair.first == pendingUid; }) 
              == studentAssignedSensors.end()) {
            logStudentToRTDB(pendingUid, timestamp, -1, "Absent", "");
          }
        }
        
        displayMessage("Verification", "Skipped by Inst.", 3000);
        displayMessage("Class Session", "Started", 3000);
        Serial.println("Verification phase skipped by instructor. Class session started.");
        logSystemEvent("Verification Phase Skipped by Instructor");
      }
